...
var block_timer;//таймер для препятствий
...
function showScore() {
    ...
    scoreDiv.innerText = "Текущий счет: " + score;
}
...
function createBlock() {
    ...
    while (!blockCreated) { 
        ...
        if (!block_cell_classes.includes('snake-unit') && !block_cell_classes.includes('food-unit')) {
            var classes = '';
   ...
   setTimeout(deleteBlock, 4000, block_cell);
}

function deleteBlock(cell) {
    var block_cell_classes = cell.getAttribute('class').split(' ');
    cell.setAttribute('class', block_cell_classes[0] + ' ' + block_cell_classes[1]);
}
